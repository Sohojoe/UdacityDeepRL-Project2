...
class MLAgentsGym(gym.Env):
    ...
    def __init__(
        ...
        if use_editor:
            ...
            marathon_envs_path = 'Reacher'
            if platform == "win32":
                marathon_envs_path = os.path.join('Reacher_Windows_x86_64', 'Reacher.exe')
    ...
    def _single_step(self, info: BrainInfo) -> GymSingleStepResult:
        if self.use_visual:
    ...
    def _multi_step(self, info: BrainInfo) -> GymMultiStepResult:
        if self.use_visual:
    ...
    def _get_vis_obs_list(self, step_result: BrainInfo) -> List[np.ndarray]:
    ...
    def _get_vector_obs(self, step_result: BrainInfo) -> np.ndarray:
        result: List[np.ndarray] = []
        for obs in step_result.obs:
            if len(obs.shape) == 2:
                result.append(obs)
    ...
    def _sanitize_info(self, step_result: BrainInfo) -> BrainInfo:
    ...
    def _step(self, needs_reset: bool = False) -> BrainInfo:
        if needs_reset:
            self._env.reset()
        else:
            self._env.step()
